...
"powershell.exe -command `"Get-NetAdapter -Physical | where {`$_.MediaType -eq 802.3} | Disable-NetAdapter -Confirm:`$false -PassThru -Verbose`" >> %WINDIR%\setupcomplete.log",
...
"powershell.exe -command `"Get-NetAdapter -Physical | where {`$_.MediaType -eq 802.3} | Enable-NetAdapter -Confirm:`$false -PassThru -Verbose`" >> %WINDIR%\setupcomplete.log",
